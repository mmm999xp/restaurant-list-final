...
//對於非必填項目的規則，若沒有收到該item，預設會通過驗證，但是若有收到該item就必須驗證，不管有沒有通過驗證都會生成該項的result
  ...
  if (object.google_map !== '') {
    if (!object.google_map.toLowerCase().includes('ftp')) {
      result7 = false
    } else if (!object.google_map.toLowerCase().includes('http')) {
      result7 = false
    } else if (!object.google_map.toLowerCase().includes('https')) {
      result7 = false
    }
  }
  //評分
  result8 = !object.rating || (typeof object.rating === 'number' && (object.rating >= 0 && object.rating <= 5))
  //描述(必填)
